...
async function config(app, { showLoginPage, isStateless }) {
  for (const idp of ['corpPass', 'singPass']) {
    app.use(express.urlencoded({ extended: true }))
    app.use(express.json())

    app.get(`/${idp.toLowerCase()}/v2/authorize`, async (req, res) => {
      const {
        scope,
        response_type,
        client_id,
      ...
      if (showLoginPage(req)) {
        ...
        const response = render(LOGIN_TEMPLATE, {
          ...
          customProfileConfig: {
            ...
            registerEndpoint: `/${idp.toLowerCase()}/v2/authorize/register`,
            showUuid: false,
    ...
    app.post(`/${idp.toLowerCase()}/v2/authorize/register`, (req, res) => {
